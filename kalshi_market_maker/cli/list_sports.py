...
def print_sports_markets(markets: List[Dict], series_filter: str = None) -> None:
    ...
    header = (
        f"{'TICKER':<36}  {'YES BID':>7}  {'YES ASK':>7}  {'VOL 24H':>9}  {'OPEN INT':>9}  TITLE"
    )
    separator = "-" * min(120, len(header) + 20)
    print(separator)
    print(header)
    print(separator)
    ...
    for market in markets:
        ...
        print(
            f"{ticker:<36}  {yes_bid:>7}  {yes_ask:>7}  {volume_24h:>9,}  {open_interest:>9,}  {display_title}"
        )
...
def main():
    parser = argparse.ArgumentParser(description="List all live Kalshi sports contracts")
    parser.add_argument(
        "--series",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--sort",
        type=str,
        choices=["ticker", "volume", "open_interest"],
        default="volume",
